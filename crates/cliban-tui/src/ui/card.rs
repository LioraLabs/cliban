...
fn priority_color(p: &str) -> Color {
    match p { "urgent" => Color::Indexed(196), "high" => Color::Indexed(208),
              "medium" => Color::Indexed(226), "low" => Color::Indexed(33), _ => Color::DarkGray }
}
fn priority_letter(p: &str) -> &'static str {
    match p { "low" => "(L)", "medium" => "(M)", "high" => "(H)", "urgent" => "(U)", _ => "( )" }
}

pub fn card_lines(card: &Card) -> (String, String) {
    (format!("{} {}", card.key, priority_letter(&card.priority)), format!("  {}", card.title))
}
...
pub fn draw_card(frame: &mut Frame, area: Rect, card: &Card, is_focused: bool, now_ms: u128) {
    ...
    } else { truncate(title_inner, viewport) };
    let lines = vec![
        Line::from(Span::styled(key_line, Style::default().add_modifier(Modifier::BOLD))),
...
fn truncate(s: &str, max: usize) -> String {
    ...
    else { let mut o: String = s.chars().take(max.saturating_sub(1)).collect(); o.push('…'); o }
}

pub fn marquee_slice(text: &str, viewport: usize, now_ms: u128) -> String {
    const STEP_MS: u128 = 200; const SEP: &str = "   •   ";
    if viewport == 0 { return String::new(); }
    let chars: Vec<char> = text.chars().collect();
    if chars.len() <= viewport { return text.to_string(); }
    let padded: Vec<char> = chars.iter().chain(SEP.chars().collect::<Vec<_>>().iter()).copied().collect();
    let len = padded.len();
...
mod tests {
    ...
    fn card(key: &str, prio: &str) -> Card {
        Card { id: 0, key: key.into(), project: "CLI".into(), title: "Hello".into(),
               status: "backlog".into(), priority: prio.into(), position: 1.0, milestone_id: None, milestone: None }
    }

    #[test]
    fn card_key_line_shows_priority_letter() {
        let (k, t) = card_lines(&card("CLI-8", "high"));
        assert_eq!(k, "CLI-8 (H)"); assert_eq!(t, "  Hello");
    }
    ...
    fn focused_border_cyan_unfocused_border_priority() {
        ...
        t.draw(|f| draw_card(f, Rect::new(0,0,30,4), &card("CLI-8","urgent"), true, 0)).unwrap();
        assert_eq!(t.backend().buffer()[(0,0)].fg, Color::Cyan);
        let mut t2 = Terminal::new(TestBackend::new(30, 4)).unwrap();
        t2.draw(|f| draw_card(f, Rect::new(0,0,30,4), &card("CLI-8","urgent"), false, 0)).unwrap();
