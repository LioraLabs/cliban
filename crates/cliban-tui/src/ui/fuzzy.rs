...
pub fn draw(frame: &mut Frame, area: Rect, app: &App, state: &FuzzyState) {
    ...
    let block = Block::default().title(" Fuzzy find ").borders(Borders::ALL)
    ...
    let chunks = Layout::default().direction(Direction::Vertical)
        .constraints([Constraint::Length(1), Constraint::Min(0), Constraint::Length(1)]).split(inner);
    ...
    let labels: Vec<String> = state.results.iter().map(|key|
        match app.cards.iter().find(|c| &c.key == key) {
            Some(c) => format!("{}  {}", c.key, c.title), None => key.clone(),
        }).collect();
    ...
    let lines: Vec<Line> = if total == 0 {
        vec![Line::styled("  (no matches)", Style::default().fg(Color::DarkGray))]
    } else {
        labels[start..end].iter().enumerate().map(|(i, label)| {
            let abs = start + i;
            if abs == cursor {
                Line::from(vec![Span::styled("▸ ", Style::default().fg(Color::Yellow)),
                    Span::styled(label.as_str(), Style::default().add_modifier(Modifier::REVERSED))])
            } else { Line::from(vec![Span::raw("  "), Span::raw(label.as_str())]) }
        }).collect()
    };
    frame.render_widget(Paragraph::new(lines), list_rect);
    let footer = Line::from(vec![
        Span::styled("Enter", Style::default().fg(Color::Green)), Span::raw(" jump  "),
