...
> **Historical — do not execute.** This is the original v1 plan, written
> 2026-05-19 against a Go implementation (Bubble Tea TUI, Cobra CLI) that no
> longer exists; cliban is Rust. It is kept for the design rationale only.
> Current work is tracked on the cliban board itself — see
> `docs/agents/issue-tracker.md`. The checkbox steps below are a record of what
> was done, not a plan to run.
