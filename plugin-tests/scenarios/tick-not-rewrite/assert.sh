...
plan=$(cliban --db "$DB" issue show "$KEY" --section plan 2>/dev/null)
...
act=$(cliban --db "$DB" issue show "$KEY" --section activity 2>/dev/null)
...
cliban --db "$DB" issue show "$KEY" --section spec >/dev/null 2>&1 \
